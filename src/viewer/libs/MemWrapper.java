...
public class MemWrapper {
	...
	public int GetByteAtMem(int memaddr) throws Exception {
		...
		return (i1);
			...
			throw new Exception("Illegal attempt to access a byte at "+ String.valueOf(realaddress)+" Actual last byte is "+String.valueOf(lastmem));
		}
	}
	...
	public int GetWordAtMem(int memaddr) throws Exception {
		...
		if (addr >= contents.length) {
			int lastmem = contents.length + ZX81_RAMSTART - P_FILE_STARTDISP;
			throw new Exception("Requested address: "+String.valueOf(memaddr)+" is outside of the PFILE.  Actual last byte is "+String.valueOf(lastmem));
		}
		byte b1 = contents[addr];
		byte b2 = contents[addr + 1];
	 ...
	 * 
	 * format is:
	 *   |Exp|MSD|Digit|Digit|LSD|
	 *   
	 *   Where EXP is the binary point location where $80 = 0
	 *   exp=0 is a special case meaning digit is 0
	 *   note the MSB of the MSD is the sign.  This is also where the most significant
	 *   digit goes, so we extract the sign, then we always treat that bit as 1.  
	 * 
	 * @param memaddr
	 * @return
	 * @throws Exception 
	...
	public double GetFloatAtMem(int memaddr) throws Exception {
		...
		if (exponent!=0) { 
			...
			fVal = value;
			//note, done like this in stages because Java treats these as 32 bit integers, its too big to do on one go. 
			fVal = fVal / 65536;
			fVal = fVal / 65536;
